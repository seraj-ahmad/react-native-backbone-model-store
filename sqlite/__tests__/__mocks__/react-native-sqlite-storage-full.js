var cordova = require('../helpers/cordova');
let navigator = {
  userAgent: 'iPhone'
}
...
SQLitePlugin = function(openargs, openSuccess, openError) {
  var dbname;
  __TEST__ && console.log("SQLitePlugin openargs: " + (JSON.stringify(openargs)));
  ...
  this.openSuccess || (this.openSuccess = function() {
    __TEST__ && console.log("DB opened: " + dbname);
  });
  this.openError || (this.openError = function(e) {
    __TEST__ && console.log(e.message);
  });
...
SQLitePlugin.prototype.addTransaction = function(t) {
  this.txQ.push(t);
  __TEST__ && console.log('TXQ length=', this.txQ.length);
  if (this.txQ.length === 1) {
    __TEST__ && console.log('starting', t);
    t.start();
  }
};

SQLitePlugin.prototype.transaction = function(fn, error, success) {
  __TEST__ && console.log('adding:', fn);
...
SQLitePlugin.prototype.executePragmaStatement = function(statement, success, error) {
  __TEST__ && console.log("SQLitePlugin::executePragmaStatement");
  pcb = success;
...
SQLitePluginCallback = {
  p1: function(id, result) {
    var mycb;
    __TEST__ && console.log("PRAGMA CB");
    mycb = pcb;
    pcb = function() {
      return 1;
    };
...
SQLitePluginTransaction.prototype.run = function() {
  ...
  mycb = function(result) {
    __TEST__ && console.log('getting called:', result);
